 ...
 * Those fields exist so a developer can point a debug build straight at TMDB without standing up
 * the backend first. In a release build the app has no embedded credentials at all and obtains
 * everything through the backend, which is the only place a production secret lives.
 *
 * TorBox / direct-provider integration has been removed. `allowDirectProvider` is permanently false.
 *
...
interface AppConfig {
    ...
    /**
     * Always false. TorBox / provider-cloud integration has been removed.
     * Kept in the interface so callers compiled against the interface do not need changes.
     */
...
/**
 * What is and is not configured, for the Diagnostics screen. Carries no secrets —
...
class BuildConfigAppConfig(
    ...
    /** TorBox removed — direct-provider mode is permanently disabled. */
    ...
    override fun status(): ConfigStatus {
        ...
        val keyHint = when (authMode) {
            TmdbAuthMode.BEARER_TOKEN  ->
                com.sustream.tv.core.log.Redact.tail(tmdbReadToken.reveal())
            TmdbAuthMode.API_KEY_QUERY ->
                com.sustream.tv.core.log.Redact.tail(tmdbApiKey.reveal())
