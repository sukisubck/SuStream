...
### 1.1 User-configured HTML/JSON manifest addons

The app can consume addons that expose an HTML or JSON manifest over HTTP. This is a **capability, not an
integration**: no addon ships with the build, there is no addon directory, no discovery, no default
URL, and none is recommended. The only addons the app will talk to are ones whose URLs the user typed in
...
| `HtmlJsonAddonPreferences.authorisedByUser` must be true | A URL alone is not enough; the user confirms entitlement first |
...
**Code:** `provider/htmljson/HtmlJsonAddonApi.kt`, `HtmlJsonAddonSourceRepository.kt`,
`Authorisation.UserAddon` in `domain/model/Playback.kt`, `HtmlJsonAddonPreferences` in
...
| **`Dimens`, never raw dp** in the design system | 960×�40 dp TV canvas; see `designsystem/theme/Dimens.kt`. |
...
| `provider/` | 9 | Source contract, IPTV-backed adapter, mock adapter, composite, TorBox + mock, the user-configured HTML/JSON-manifest addon adapter (§1.1) and its setup-time manifest probe |
...
| 5a | **Addon settings panel** — part of Settings | `SettingsRepository` (`HtmlJsonAddonPreferences`), `AddonManifestProbe` | **In progress — step 1 of 3 done** (logic). Remaining: the screen itself, a route to reach it, and strings; then a probe unit test. See "5a progress" below. |
...
- `provider/htmljson/AddonManifestProbe.kt` — fetches an addon's `manifest.json` and reports back
...
- `normaliseAddonBaseUrl()` — accepts `https://host`, `https://host/`, `https://host/manifest.json`
  and a config-path variant. **Shared with `HtmlJsonAddonSourceRepository`**, which previously had its
...
**Still open, decide before step 2:** `HtmlJsonAddonPreferences` holds **one** addon. Supporting
...
- `HtmlJsonAddonSourceRepositoryTest` — **the constraint tests from §1.1, which are the ones that
